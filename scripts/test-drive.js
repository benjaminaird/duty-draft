...
async function main() {
  ...
  try {
    ...
    const report = [
      ...
      '',
      '## Scope\n\n- Current run: 12-month simulation, with detailed report shown for month 1\n- Next planned upgrade: 12-month loop with carry-forward fairness history\n\n## Server Check',
      '',
      `- Test mode: ON`,
      `- Test API: ${BASE_URL}`,
      ...
      `- Months completed: ${monthResults.filter(r=>r.draftState.draftDone).length}/${monthResults.length}`,
      `- Total duty assignments simulated: ${monthResults.reduce((sum,r)=>sum+Object.keys(r.draftState.assignments||{}).length,0)}`,
      `- Total weekend assignments simulated: ${monthResults.reduce((sum,r)=>sum+Object.entries(r.draftState.assignments||{}).filter(([d])=>isWkDate(Number(d),r.draftState)).length,0)}`,
      `- Months with complete drafts: ${monthResults.every(r=>r.draftState.draftDone) ? 'PASS' : 'FAIL'}`,
      "",
